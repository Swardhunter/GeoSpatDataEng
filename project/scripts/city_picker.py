...
class PickerHandler(SimpleHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        # Save selection and stop the server; fetching runs in main() afterwards.
        self.server.selected_city = city
...
def main():
    ...
    print(f"  → {len(FETCH_SCRIPTS)} fetch scripts run, then server exits.\n")
    webbrowser.open(url)
    server.serve_forever()
    server.server_close()

    if not server.selected_city:
        return

    print(f"\n{'='*50}")
    print(f"  Fetching data for {server.selected_city}...")
    print(f"{'='*50}\n")
